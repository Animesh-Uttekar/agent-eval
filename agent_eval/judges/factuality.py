...
class FactualityJudge(BaseJudge):
    """
    Judge for evaluating the factual accuracy of the model's output.
    """

    aliases = ["factuality", "factual", "factuality_judge", "factuality-judge"]

    def __init__(self, model, criteria="factuality", provider=None):
        super().__init__(model, criteria, provider)
        self.prompt_template = DEFAULT_FACTUALITY_PROMPT
